...
export default function VideoExperiment({ 
  ...
  useEffect(() => {
    if (initialVideoData) {
      ...
      console.log("[VIDEO RESTORED]", initialVideoData.video_id, initialVideoData.video_url)
    } else {
      // 新开始实验，随机选择视频
      const video = getRandomVideo()
      setVideoData(video)
      console.log("[VIDEO PICKED]", video.video_id)
      console.log("[VIDEO URL]", video.video_url)
      
      // 通知父组件视频数据已准备好（用于创建 session）
      if (onVideoDataReady && video) {
        onVideoDataReady(video)
      }
    }
  }, [initialVideoData, onVideoDataReady])
  ...
  useEffect(() => {
    ...
    console.log('开始加载视频:', videoData.url)
    ...
    const handleLoadStart = () => {
      console.log('视频开始加载...', videoData.url)
      setIsVideoLoading(true)
    }
    ...
    // 视频可以播放
      const handleCanPlay = () => {
      console.log('视频可以播放', videoData.url)
  ...
  }, [videoData, onComplete, isVideoLoading])
