...
export default function Questionnaire({ 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 写入前打印最终 payload（确认不再包含 return_count 和 watch_time_ms）
      console.log('📦 [FINAL PAYLOAD] 写入 study3_data 的数据：', JSON.stringify(payload, null, 2))
      console.log('📦 [VERIFY] return_count 已移除:', !payload.hasOwnProperty('return_count'))
      console.log('📦 [VERIFY] watch_time_ms 已移除:', !payload.hasOwnProperty('watch_time_ms'))
      console.log('📦 [VERIFY] watch_time 存在且为秒:', payload.watch_time, '秒')
